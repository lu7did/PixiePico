...
## Diagnóstico USB CDC-only

Esta copia compila por defecto solamente la clase USB CDC. La interfaz UAC2
Audio, sus endpoints y sus callbacks quedan fuera del firmware, mientras que
`main.c` conserva la misma API mediante funciones neutras.

Debe configurarse desde un directorio `build` nuevo para evitar reutilizar la
caché de CMake de una compilación anterior.

Para volver a incluir USB Audio, configure CMake con
`-DPIXIE_ENABLE_USB_AUDIO=ON`. Para regresar al diagnóstico CDC-only, utilice
`-DPIXIE_ENABLE_USB_AUDIO=OFF`.
